...
def load_tf_weights_in_bert(model, tf_checkpoint_path):
    ...
    try:
        ...
        print(
            "Loading a TensorFlow backbone in PyTorch, requires TensorFlow to be installed. Please see "
...
class BertConfig(object):
    ...
    def __init__(
        ...
        v_feature_size=1024,  #  v_feature_size=2048,
        v_target_size=1601,
        v_hidden_size=768,
...
class BertEmbeddingLocation(nn.Module):
    def __init__(self, config, params):
        super(BertEmbeddingLocation, self).__init__()
        self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size)
        self.position_embeddings = nn.Embedding(config.max_position_embeddings, config.hidden_size)
        self.txt_location_embeddings = nn.Linear(4, config.hidden_size)
        # ----- uncomment for good weights ---
        ...
        # -------
        self.plotqa_type_embeddings = nn.Embedding(config.plotqa_vocab_types, config.hidden_size)
    ...
    def forward(self, input_ids, sep_indices=None, sep_len=None, token_type_ids=None, loc=None):
        ...
        # consider only Q and A:
        position_ids[(token_type_ids != -1) & (token_type_ids != 1)] = input_ids.shape[1]
        position_ids = position_ids - torch.min(position_ids, dim=-1)[0].unsqueeze(1)
        position_ids[(token_type_ids != -1) & (token_type_ids != 1)] = 0

        position_embeddings = self.position_embeddings(position_ids)
        position_embeddings[(token_type_ids != -1) & (token_type_ids != 1)] = 0
        words_embeddings = self.word_embeddings(input_ids)

        # new_embeddings = []
        # for i in range(words_embeddings.shape[0]):
        #     vectors = [words_embeddings[i][0]]
        #     prev = 1
        #     for j in torch.where(input_ids[i] == 102)[0]:
        #         vectors.append(torch.sum(words_embeddings[i][prev: j], dim=0))
        #         prev = j

        loc_embeddings = self.txt_location_embeddings(loc)
        loc_embeddings[torch.sum(torch.abs(loc), dim=-1) == 0] = 0
        # zero <cls> type embeddings + padding embeddings
        q_to_zero = token_type_ids.clone()
        q_to_zero[q_to_zero == -1] = 0
        token_type_embeddings = self.plotqa_type_embeddings(q_to_zero)
        token_type_embeddings[token_type_ids == 0] = 0
        # -----
        embeddings = words_embeddings + position_embeddings + token_type_embeddings + loc_embeddings
...
class BertImageSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, image_loc=None):
        mixed_query_layer = self.query(hidden_states)
        mixed_key_layer = self.key(hidden_states)
        mixed_value_layer = self.value(hidden_states)
...
class BertImageAttention(nn.Module):
    def __init__(self, config, params):
        ...
        self.self = BertImageSelfAttention(config, params)
        self.output = BertImageSelfOutput(config)

    def forward(self, input_tensor, attention_mask, image_loc=None):
        self_output, attention_probs = self.self(input_tensor, attention_mask, image_loc=image_loc)
        attention_output = self.output(self_output, input_tensor)
        return attention_output, attention_probs
...
class BertImageLayer(nn.Module):
    def __init__(self, config, params):
        ...
        self.attention = BertImageAttention(config, params)
        self.intermediate = BertImageIntermediate(config)
        self.output = BertImageOutput(config)

    def forward(self, hidden_states, attention_mask, image_loc=None):
        attention_output, attention_probs = self.attention(hidden_states, attention_mask, image_loc=image_loc)
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(intermediate_output, attention_output)
        return layer_output, attention_probs
...
class BertEncoder(nn.Module):
    def __init__(self, config, params):
        ...
        v_layer = BertImageLayer(config, params)
    ...
    def forward(
        ...
        skip_txt, skip_img = txt_embedding, image_embedding
        ...
        for v_layer_id, t_layer_id in zip(self.v_biattention_id, self.t_biattention_id):
            ...
            for idx in range(v_start, v_end):
                image_embedding, image_attention_probs = self.v_layer[idx](image_embedding,
                                                                           image_attention_mask,
                                                                           image_loc=image_loc)
            ...
            if skip_connections:
                txt_embedding += skip_txt
                image_embedding += skip_img

            if output_all_encoded_layers:
                all_encoder_layers_t.append(txt_embedding)
                all_encoder_layers_v.append(image_embedding)
            elif output_intermediate_layers is not None:
                if count in output_intermediate_layers:
                    all_encoder_layers_t.append(txt_embedding)
                    all_encoder_layers_v.append(image_embedding)
...
class BertPreTrainingHeads(nn.Module):
    def __init__(self, config, bert_model_embedding_weights, params):
    ...
    def forward(
        ...
        return prediction_scores_t, None, seq_relationship_score
...
class BertPreTrainedModel(nn.Module):
    """ An abstract class to handle weights initialization and
        a simple interface for dowloading and loading pretrained backbone.
    """
    ...
    def from_pretrained(
        ...
        **kwargs
        ...
        Params:
            ...
            cache_dir: an optional path to a folder in which the pre-trained backbone will be cached.
            state_dict: an optional state dictionnary (collections.OrderedDict object) to use instead of Google pre-trained backbone
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, params):
        ...
        self.embeddings = BertEmbeddingLocation(config, params)

        # initlize the vision embedding
        self.v_embeddings = BertImageEmbeddings(config, params)
    ...
    def forward(
        ...
        # vis_belong, txt_belong = legend_belonging
        # embedding_output = self.embeddings(input_txt, token_type_ids=token_type_ids, sep_indices=sep_indices, sep_len=sep_len, loc=txt_loc, txt_belong=txt_belong)
        # v_embedding_output = self.v_embeddings(input_imgs, image_loc, image_target, vis_belong=vis_belong)
        embedding_output = self.embeddings(input_txt, token_type_ids=token_type_ids, sep_indices=sep_indices, sep_len=sep_len, loc=txt_loc)
        v_embedding_output = self.v_embeddings(input_imgs, image_loc, image_target, areas)
        ...
        encoded_layers_t, encoded_layers_v, all_attention_mask = self.encoder(
            ...
            output_intermediate_layers=output_intermediate_layers,
            skip_connections=False,
            image_loc=image_loc
        )
        ...
        if output_intermediate_layers is not None:
...
class BertImageEmbeddings(nn.Module):
    ...
    def __init__(self, config, params):
        ...
        # self.trans_1 = nn.Linear(1024, config.v_feature_size)
        self.new_image_embeddings = nn.Linear(config.v_feature_size, config.v_hidden_size)
        # self.image_location_embeddings = nn.Linear(5, config.v_hidden_size)
        # self.trans_2 = nn.Linear(4, 5)
        # -------
        # self.fig_type = nn.Embedding(4, config.v_hidden_size)
        self.params = params
        self.type_embeddings = nn.Embedding(13, config.v_hidden_size)

        self.color_emb = nn.Embedding(params['categories'] + 1, config.v_hidden_size)
        self.new_loc_emb = nn.Linear(4, config.v_hidden_size)

        if params['dataset'] in ['figure_qa', 'dvqa']:
            self.areas_emp = nn.Linear(1, config.v_hidden_size)
        if params['dataset'] == 'figure_qa':
            self.sep_emb = nn.Embedding(params['max_previews'] + 1, config.v_hidden_size)

        self.LayerNorm = BertLayerNorm(config.v_hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.b_hidden_size = config.v_hidden_size
        # self._____counter = 0

    def forward(self, input_ids, input_loc, input_type, areas=None):
        # img_embeddings = self.image_embeddings(self.trans_1(input_ids))
        img_embeddings = self.new_image_embeddings(F.softmax(input_ids, dim=-1))

        loc_embeddings = self.new_loc_emb(input_loc)
        color_embeddings = self.color_emb(input_type)
        if self.params['dataset'] in ['figure_qa', 'dvqa']:
            # color_embeddings = self.sep_emb(input_type)
            sum_embs = loc_embeddings
        else:
            sum_embs = img_embeddings + loc_embeddings

        sum_embs += color_embeddings

        if areas is not None:
            sum_embs += self.areas_emp(areas)

        if self.training and self.params["mask_prob_img"] > 0:
            mask = torch.rand(sum_embs.shape[:2]) < self.params["mask_prob_img"]
            sum_embs[mask] = 0
        embeddings = self.LayerNorm(sum_embs)
        embeddings = self.dropout(embeddings)
        return embeddings
...
class BertForMultiModalPreTraining(BertPreTrainedModel):
    ...
    def __init__(self, config, params=None):
        super(BertForMultiModalPreTraining, self).__init__(config, params)
        self.params = params
        self.bert = BertModel(config, params)
        self.cls = BertPreTrainingHeads(
            config, self.bert.embeddings.word_embeddings.weight, self.params
        )
        ...
        # if self.params['dataset'] in ['plotqa', 'plotqa_colorless', 'dvqa']:
        if not self.params['binary_answers'] and (self.params['dataset'] != 'dvqa' or '_cls' not in self.params['qa_file']):
            if self.params['CE_REG']:
                self.regressor = DVQA_Regressor_v20_CE(config)
                self.regressor_loss_func = torch.nn.CrossEntropyLoss(reduction='none')
            else:
                self.regressor = PlotQA_Regressor_v20(config)

                if params['L1']:
                    self.regressor_loss_func = torch.nn.L1Loss(reduction='none')
                else:
                    self.regressor_loss_func = torch.nn.SmoothL1Loss(reduction='none', beta=0.5)

            if self.params['rank'] == self.params['rank_from'] and not self.params['BOT_MODE']:
                print("-"*20, f"Regressor: {self.regressor.__class__.__name__}", "-"*20)
                print("-"*20, f"Loss: {str(self.regressor_loss_func)}", "-"*20)
                print("-"*20, f"MAX_SEQ_LEN: {params['max_seq_len']}, max_vis_features: {params['max_vis_features']}", "-"*20)

            # self.loc_loss = PlotQA_location_loss(config)

            self.regressor_loss_func_l1 = torch.nn.L1Loss(reduction='none')
    ...
    def forward(
        ...
        prediction_scores_t, _, seq_relationship_score = self.cls(sequence_output_t, sequence_output_v,
                                                                  pooled_output_t, pooled_output_v)

        # ====== legend loss ====
        legend_loss = torch.cuda.FloatTensor([0], device=self.params['device']) if self.params['device'].type != 'cpu' else torch.FloatTensor([0])

        # ====== Regressor ======
        gt_reg, reg_loss_kind = gt_reg[0], gt_reg[1]
        # -----
        needs_regression = (gt_reg[:, 1] == True)

        reg = [torch.zeros(gt_reg.shape[0], device=self.params['device'], dtype=gt_reg.dtype),
               torch.zeros(gt_reg.shape[0], device=self.params['device'], dtype=gt_reg.dtype),
               torch.zeros(gt_reg.shape[0], device=self.params['device'], dtype=gt_reg.dtype),
               (0, 0),
               torch.zeros(gt_reg.shape[0], device=self.params['device'], dtype=gt_reg.dtype),
               ]

        number_of_regressions = torch.sum(needs_regression)
        if number_of_regressions > 0 or self.params['BOT_MODE']:
            regression = self.regressor(hv_0=sequence_output_v[needs_regression][:, 0],
                                        hw_0=sequence_output_t[needs_regression][:, 0],
                                        )

            if self.params['CE_REG']:
                reg_targets = gt_reg[needs_regression][:, 0].long()

                reg_targets_vals = torch.tensor(self.params['dvqa_floats'], device=self.params['device'])[reg_targets]
                reg_loss = self.regressor_loss_func(regression, reg_targets)
                chosen_idx = torch.argmax(regression, dim=-1)
                reg[0][needs_regression] = torch.tensor(self.params['dvqa_floats'], device=self.params['device'])[chosen_idx]
                reg[1][needs_regression] = reg_loss
                reg[2][needs_regression] = self.regressor_loss_func_l1(reg[0][needs_regression], reg_targets_vals)
                correct_regs = correct_t_regs = (chosen_idx == reg_targets)

                reg[3] = torch.sum(correct_regs).item(), torch.sum(correct_t_regs).item()
                reg[4][needs_regression] = reg[2][needs_regression]
            else:
                reg_targets = gt_reg[needs_regression][:, 0] / gt_reg[needs_regression][:, 3]

                if self.params['dataset'] == 'dvqa':
                    # clipping for DVQA
                    if not self.training:
                        regression *= gt_reg[needs_regression][:, 3]
                        for i in range(len(regression)):
                            regression[i] = min(self.params['dvqa_floats'], key=lambda x:abs(x-regression[i]))
                        regression /= gt_reg[needs_regression][:, 3]

                reg_loss = self.regressor_loss_func(regression, reg_targets)
                reg_l1 = self.regressor_loss_func_l1(regression, reg_targets)

                # correct when +-5%, or target & pred are 0
                #  +-5% dist equivalent: (|(r0-r1)| / |r0|) <= 0.05
                reg_5_dist = reg_l1 / torch.abs(reg_targets)
                reg_5_dist[reg_targets == 0] = 1
                reg_5_dist[(regression == 0) & (reg_targets == 0)] = 0

                correct_regs = (reg_5_dist <= 0.05) | ((regression == 0) & (reg_targets == 0))
                correct_t_regs = (reg_l1 <= self.params['tol_margin'])

                if reg_loss_kind != 'L1':
                    # zero gradients of impossible answers
                    reg_loss[torch.abs(reg_targets) > 1] = 0
                    # ---

                reg[0][needs_regression] = regression * gt_reg[needs_regression][:, 3]
                reg[1][needs_regression] = reg_loss
                reg[2][needs_regression] = reg_l1
                reg[3] = torch.sum(correct_regs).item(), torch.sum(correct_t_regs).item()
                reg[4][needs_regression] = reg_5_dist

        # =======================
        if masked_lm_labels is not None and next_sentence_label is not None and image_target is not None:
            masked_img_loss = torch.cuda.FloatTensor([0], device=self.params['device']).unsqueeze(0) if self.params['device'].type != 'cpu' else torch.FloatTensor([0]).unsqueeze(0)
            masked_lm_loss = torch.cuda.FloatTensor([0], device=self.params['device']).unsqueeze(0) if self.params['device'].type != 'cpu' else torch.FloatTensor([0]).unsqueeze(0)

            next_sentence_loss = self.loss_fct(
                seq_relationship_score.view(-1, 2), next_sentence_label.view(-1)
            )
            # total_loss = masked_lm_loss + next_sentence_loss + masked_img_loss
            return masked_lm_loss, masked_img_loss, next_sentence_loss.unsqueeze(0), sequence_output_t, prediction_scores_t, seq_relationship_score, reg, legend_loss
        else:
            return prediction_scores_t, None, seq_relationship_score, sequence_output_t, all_attention_mask, reg, legend_loss
